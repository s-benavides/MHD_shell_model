...
nf = 0 # Forcing wavenumber
N = 29+nf # Number of wavenumbers 
...
eps1 = 0 # energy injection rate of u0 field
u0_init = 10**(-2.) #1e0
u1_init = 0.0 #1e0
nu = 2e-7 # These are references from literature, not actual values. Check code
nu_l = 0.0
# For later use? 
Q = 0
...
NSc_3 = 20 # Iterations between scalar saves (could be more frequent, for flux and thetas averages). If NSc_freq = np.nan, then it doesn't output this at all.
